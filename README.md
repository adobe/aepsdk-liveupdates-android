...
- `setLiveUpdateListener` / `getLiveUpdateListener` — register an `ILiveUpdateListener` for `onLiveUpdateReceived` / `onStart` / `onUpdate` / `onEnd` callbacks.
...
This SDK uses **commons 4.0.0** (AGP 8.9.1 / Kotlin 2.0.21 / compileSdk 36 / Gradle 8.11.1), which is a newer toolchain than the rest of the AEP Android SDK family.
...
| Dependency | Source |
|---|---|
| `aep-library` Gradle plugin (commons 4.0.0) | JitPack, tracking `aepsdk-commons`'s `dev-v4.0.0-SNAPSHOT` |
